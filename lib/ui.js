...
function UI(){
    ...
    this.createButton = function(){
        ...
        CustomizableUI.addListener({
            onWidgetBeforeDOMChange: function(node, nextNode, container, getRemoved){
                ...
                node.addEventListener('click', notifier.buttonClick.bind(notifier), true);
                
                let menu = self.buildContextMenu(node);
                node.appendChild(menu.menuPopup);
                
                node.addEventListener('contextmenu', function(e){
                    e.preventDefault();
                    e.stopPropagation();
                    self.showContextMenu(menu.menuPopup, node);
                });
            }
        });
    ...
    this.buildContextMenu = function(node){
        ...
        this.addChild(menuPopup, item, 'Refresh', null, notifier.fetch.bind(notifier));
        menuPopup.appendChild(menuSeparator.cloneNode(true));
        this.addChild(menuPopup, item, 'Settings', null, this.showSettings);
    ...
    this.addChild = function(menuPopup, item, label, value, command){
        var item_ = item.cloneNode(true);
        item_.setAttribute('label', label);
        item_.setAttribute('value', value);
